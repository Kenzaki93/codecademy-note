...
--Finally, between the curly braces {} is the function body, or the JavaScript statements that define the function. This is followed by a semi-colon ;. 
In JavaScript, any code between curly braces is also known as a block.
...
Let's explore how this function works:
        ...
        3) When we call the multiplyByThirteen() function on the last line, we set the inputNumber parameter. 
           Here, we set it to 9. Then, inside the function block, 9 is multiplied by 13, resulting in 117 printing to the console.

        4) Note on terminology: inputNumber is a parameter, but when we call multiplyByThirteen(9), the 9 is called an argument. 
...
const Order = (banana) => {
    console.log('Order: pizza topped with ' + banana);
  };
...
3) When getAverage() runs, the function knows what numberOne and numberTwo equal since we passed in two arguments when we called the function. 
...
const taketheOrder = (topping, crustType) => {
    console.log('Order: ' + crustType + ' pizza topped with ' + topping);
  };
  
  taketheOrder('mushroom', 'thin crust');
  taketheOrder('spinach', 'whole wheat');
  taketheOrder('pepperoni', 'brooklyn style');
...
const getAveraged = (numberOne, numberTwo) => {
  const averaged = (numberOne + numberTwo) / 2;
  return averaged;
}

console.log(getAveraged(365, 27));
// Output: 196
/*
...
const takeOrdered = (topping, crustType) => {
  orderCounted++;
  console.log('Order: ' + crustType + ' pizza topped with ' + topping);
};

takeOrdered('mushroom', 'thin crust');
takeOrdered('spinach', 'whole wheat');
takeOrdered('pepperoni', 'brooklyn style');

const getSubTotal = (itemCount) => {
  return itemCount * 7.5;
};

console.log(getSubTotal(orderCounted));
//----------------------------------------------------------------------------------------------------------------------------------------------
                /*Return II
...
The multiplyByNineFifths() function multiplied the celsius by (9/5). Then it returned its value so the getFahrenheit() function could continue on to add 32 to it.
...
const takeOrderr = (topping, crustType) => {
  orderCountt++;
  console.log('Order: ' + crustType + ' pizza topped with ' + topping);
};

takeOrderr('mushroom', 'thin crust');
takeOrderr('spinach', 'whole wheat');
takeOrderr('pepperoni', 'brooklyn style');

const getSubTotall = (itemCount) => {
  return itemCount * 7.5;
};

const getTax = () => {
 return getSubTotall(orderCountt) * 0.06; 
};

const getTotall = () => {
  return getSubTotall(orderCountt) + getTax(); 
}; 

console.log(getSubTotall(orderCountt));
...
// Output: 25.
/*
Function declarations require the keyword function, a name, and a function body. You can identify this by the use of function square() and the {} below. 
...
const isGreaterrThan = (numberOne, numberTwo) => {
    if(numberOne > numberTwo){
      return true;
    } else {
      return false;
...
We'll explore a few of these techniques here, using an example function from a previous exercise.*/

const multiplyByNineFifthss = (celsiuss) => {
  return celsiuss * (9/5);
};

const getFahrenheitt = (celsiuss) => {
  return multiplyByNineFifthss(celsiuss) + 32;
};

/*console.log('The temperature is ' + getFahrenheit(15) + '°F');
We can refactor this function in three ways. The most condensed form of the function is known as concise body.

1) Functions that take a single parameter should not use parentheses. The code will still work, but it's better practice to omit the parentheses around single parameters. 
...
const multiplyByNineFiftths = cellsius => cellsius * (9/5);

const getFahrenhheit = cellsius => multiplyByNineFiftths(cellsius) + 32;

console.log('The temperature is ' + getFahrenhheit(15) + '°F');
...
2) Functions take data, perform a set of tasks on the data, and then return the result.
3) We can define parameters to be used when calling the function.
